...
<body>
   <div class="container"><!--contenedor de menu-->
    ...
    echo '
      	...
      	<b>Precio de la suscripcion : </b><input class="form-control" id="campoDeshabilitado" type="text" value="'.$precio.'" disabled><br><br><br>';
        ?>
        <?php
        $variable = $preference['response']['sandbox_init_point'];
        echo '<img src="php/ejemploQr.php?qr='.$variable.'"/>';
        ?>
        <a class="btn btn-default btn-lg btn-block" data-id2="<?php echo $id_edicion?>"id="suscripcion" href="<?php echo $preference['response']['sandbox_init_point']; ?>">Pagar</a>
        </div>
       </div>
      </section>
   ...
   ?>
     ...
     <script>
        ...
        $(document).on('click', '#suscripcion', function(){  
           ...
           {  
                $.ajax({  
                     url:"php/suscripcion_edicion.php",  
                     method:"POST",  
